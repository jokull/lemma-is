...
- **"á" after pronoun subject resolves to `eiga`** (Disambiguator): the `after_pronoun` preference rule was structurally dead — it matched the preferred candidate on `lemma === "á" && pos === "so"`, but the verb reading's lemma is `eiga`, so it never fired and "Ég á bíl" / "Hann á hest" indexed the preposition. Rules now support `preferLemma` ("á" → "eiga"); the SVO rule accepts pronoun subjects (Ég, Hann, Hún carry `fn` + `nf` morph) instead of requiring a noun reading, with a noun-only case fallback ("mig" stays accusative); and a verb-dominant previous word (BÍN noun homographs of "fer", "búa") no longer masquerades as a subject — fixing "Ég fer á fjallið" → `á` as a bonus. The `before_noun` rule stays dead by design: reviving it (prefer preposition before any capitalized object) regresses "Barnið á Leikfangið", and preposition+case government already covers its cases. Closes #12.
